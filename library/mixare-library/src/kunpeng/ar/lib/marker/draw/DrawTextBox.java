 ...
 */
package kunpeng.ar.lib.marker.draw;
...
public class DrawTextBox extends DrawCommand {
	...
	public static DrawTextBox init(Parcel in){
		...
		return new DrawTextBox(visible, distance, title, underline, (TextObj)textObjHolder.getObject(),
								(Label)textLabholder.getObject(), (ARVector)signMarkerHolder.getObject() );
	}
	
	public DrawTextBox(Boolean visible, Double distance, String title, Boolean underline, TextObj textblock, Label textlab, ARVector signMarker){
		super(CLASS_NAME);
		...
		setProperty(PROPERTY_NAME_TEXTBLOCK, new ParcelableProperty("kunpeng.ar.lib.gui.TextObj", textblock));
		setProperty(PROPERTY_NAME_TEXTLAB, new ParcelableProperty("kunpeng.ar.lib.gui.Label", textlab ));
		setProperty(PROPERTY_NAME_SIGNMARKER, new ParcelableProperty("kunpeng.ar.lib.render.MixVector", signMarker));
	}
	...
	public void draw(PaintScreen dw) {
		...
		ARVector signMarker = getMixVectorProperty(PROPERTY_NAME_SIGNMARKER);
