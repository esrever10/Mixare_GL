...
import kunpeng.ar.plugin.PluginLoader;
...
/**
 * This is the plugin loading activity for AR Navigator. This activity will load a splashscreen and then initializes the PluginLoader
 * It will then launch the visible bootstrap plugins and waits for their results. After all bootstrap plugins are loaded
 * then AR Navigator will be launched.
 * @author A.Egal
 */
public class PluginLoaderActivity extends Activity {
	...
	private void startMixare() {
		if (arePendingActivitiesFinished()) {
			startActivity(new Intent(this, ARView.class));
			finish();
		}
	}
