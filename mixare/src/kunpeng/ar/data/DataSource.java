...
public class DataSource extends Activity {
	...
	public DataSource(String name, String url, TYPE type, DISPLAY display,
		...
		Log.d("AR Navigator", "New Datasource!" + name + " " + url + " " + type + " "
				+ display + " " + enabled);
	}
	...
	public String createRequestParams(double lat, double lon, double alt,
			float radius, String locale, String query) {
		String ret = "";
		if (!ret.startsWith("file://")) {
			...
			case WIKIPEDIA:
				...
				ret += "?lat=" + lat + "&lng=" + lon + "&radius="
						...
						+ "&username=AR Navigator";
				break;
			case BAIDU:
				ret += "&query=" + query + "&location=" + lat + ","
						+ lon + "&radius="+ Float.toString(radius*1000) + "&output=json" 
						+ "&key=b93c8452d69f71231c9b18b7881637b7";
				break;
			default:
				break;
			}
